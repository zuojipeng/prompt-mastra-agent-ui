...
| Preview deployment trust boundary and judge procedure are reviewable | PARTIAL CLOUD | `preview-runtime-decision.md`, threat model, judge runbook, `preview-deployment-result.json` | Access and rollback are proven; the 2026-07-26 attempt reached Cloudflare but returned HTTP 302 before the Pages Function, so cloud B2 and judge path remain blocked |
