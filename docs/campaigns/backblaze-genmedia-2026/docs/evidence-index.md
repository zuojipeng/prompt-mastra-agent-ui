...
| Cloudflare provenance runtime is source-bound and reproducible locally | LOCAL WORKER | `preview-runtime-plan.json`, same-origin Pages Function, exact retained-source binding, SigV4 lock, runtime validator, focused Worker tests | No Pages binding, Access policy, rate rule, or live B2 Worker request is yet evidenced |
| Preview cloud configuration, smoke, and rollback handoff are machine checked | DESIGN ONLY | `preview-deployment-packet.json`, packet validator, substitution/secret/authority negative tests | Two B2 secret fields and release commit remain null; Access, rate limit, deployment, and post-deploy smoke are not configured |
