...
| Failure and retry preserve parent lineage | PROVEN | fixture transport, project receipt contract, unit and desktop/mobile E2E assertions | Full run remains session-only; project persistence contains hashes and lineage but no object URLs |
| Sanitized provenance receipt survives project restore and reload | PROVEN | `lib/project-workspace.ts`, strict receipt tests, desktop/mobile Playwright assertions | Receipt is a non-secret digest summary, not the complete provider/B2 record |
...
| Preview deployment trust boundary and judge procedure are reviewable | PARTIAL CLOUD | `preview-runtime-decision.md`, threat model, judge runbook, Access smoke preflight plan, `preview-deployment-result.json`, `2026-07-26-cloudflare-access-readonly-diagnosis.md` | Access and rollback are proven; the latest identity-only attempt had a persisted attachment but returned HTTP 302 before the Pages Function. Read-only logs narrow the failure to an unaccepted service-token request without proving its exact credential/header subtype |
| Cloudflare provenance runtime is source-bound and reproducible locally | LOCAL B2 + GUARDED CLOUD DEPLOYMENT | `preview-runtime-plan.json`, same-origin Pages Function, exact retained-source binding, focused Worker tests, `2026-07-23-cloudflare-preview-deployment-smoke.md`, `2026-07-26-cloudflare-access-identity-health-preflight.md` | Deployment succeeded and equivalent local B2 passed previously; the latest identity-only health request stopped at Access despite a persisted policy attachment, and no new business request ran |
| Preview cloud configuration, smoke, and rollback handoff are machine checked | DEPLOYED / FUNCTION UNREACHED | `preview-deployment-packet.json`, `preview-deployment-result.json`, packet validator, deployment smoke reports | Pinned deployment and temporary Access cleanup passed; one no-retry POST returned HTTP 302 before the Pages Function |
| Preview runtime can preserve reviewed Runway lineage through private B2 evidence | LOCAL B2 + UNVERIFIED CLOUD | `b2_preview_executor.py`, server-only source binding, injected-backend tests, local B2 result, protected deployment attempt | Equivalent local transaction passed; the authorized cloud attempt ended at DNS before HTTP |
| Reviewed source promotion has a no-overwrite core, dedicated approval/result contract, and one retained private B2 source | PROVEN | `preview_source_promotion.py`, `preview_source_promotion_contract.py`, guarded adapter, private mode-0600 approval/marker/result, exact read-back SHA-256 | One 1,044,064-byte object at one approved private key; no public serving, deployment, publication, submission, or repeatability claim |
...
| Guarded retained-source adapter implements the reviewed lifecycle | PROVEN | `guarded_preview_source_live_adapter.py`, least-privilege scope attestation, durable approval marker, private live result, 7 focused adapter tests | Exact default B2 factory was used once with a process-local `total_max_attempts=1` override; future runtime no-retry behavior is not yet code-frozen |
