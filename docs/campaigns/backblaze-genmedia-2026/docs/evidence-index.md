...
| B2 credential scope is deny-by-default and traceable in retained-source evidence | BLOCKED | `b2_credential_scope_attestation.py`, guarded adapter, 37 focused tests, private mode-0600 live scope inspection | Live key target is correct but its broad preset includes bucket configuration writes; no passing attestation or execution authority exists |
