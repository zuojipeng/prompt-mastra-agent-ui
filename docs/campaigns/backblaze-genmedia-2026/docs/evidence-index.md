...
| Preview deployment trust boundary and judge procedure are reviewable | PARTIAL CLOUD | `preview-runtime-decision.md`, threat model, judge runbook, Access smoke preflight plan, `preview-deployment-result.json` | Access and rollback are proven; the 2026-07-26 attempt reached Cloudflare but returned HTTP 302 before the Pages Function. A no-network attachment attestation contract now prevents another business POST from being attempted on zero-usage or one-way policy evidence, but no new cloud verification has occurred |
| Cloudflare provenance runtime is source-bound and reproducible locally | LOCAL B2 + GUARDED CLOUD DEPLOYMENT | `preview-runtime-plan.json`, same-origin Pages Function, exact retained-source binding, focused Worker tests, `2026-07-23-cloudflare-preview-deployment-smoke.md`, `2026-07-26-cloudflare-authenticated-smoke-attempt.md` | Deployment succeeded and equivalent local B2 passed previously; the latest Service Auth policy attachment did not persist, so the request stopped at Access |
