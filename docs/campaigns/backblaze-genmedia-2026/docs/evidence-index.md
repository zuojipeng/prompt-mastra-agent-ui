...
| B2 credential scope is deny-by-default and traceable in retained-source evidence | LOCAL ONLY | `b2_credential_scope_attestation.py`, guarded adapter, 37 focused tests, credential-scope decision | Contract and fixtures only; real configured key scope has not been inspected and the record cannot authorize execution |
