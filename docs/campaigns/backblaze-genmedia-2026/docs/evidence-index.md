...
| Railway provenance runtime is packaged, source-bound, and reproducible locally | LOCAL CONTAINER | `preview-runtime-plan.json`, exact retained-source binding, tracked no-retry B2 backend, Railway config, pinned Dockerfile/dependency lock, runtime validator, container smoke report | No Railway resource, variable, domain, secret configuration, or deployment exists |
| Preview cloud configuration, smoke, and rollback handoff are machine checked | DESIGN ONLY | `preview-deployment-packet.json`, packet validator, substitution/secret/authority negative tests | Four secret fields and release commit remain null; Access, rate limit, cloud resources, deployment, and post-deploy smoke are not configured |
