...
| Preview deployment trust boundary and judge procedure are reviewable | PARTIAL CLOUD | `preview-runtime-decision.md`, threat model, judge runbook, `preview-deployment-result.json` | Access and rollback are proven; the 2026-07-24 authenticated attempt ended at local DNS before HTTP, so cloud B2 and judge path remain blocked |
| Cloudflare provenance runtime is source-bound and reproducible locally | LOCAL B2 + GUARDED CLOUD DEPLOYMENT | `preview-runtime-plan.json`, same-origin Pages Function, exact retained-source binding, focused Worker tests, `2026-07-23-cloudflare-preview-deployment-smoke.md`, `2026-07-24-cloudflare-authenticated-smoke-attempt.md` | Deployment succeeded and equivalent local B2 passed previously; latest cloud smoke was unreached because DNS failed before connection |
| Preview cloud configuration, smoke, and rollback handoff are machine checked | DEPLOYED / SMOKE UNREACHED | `preview-deployment-packet.json`, `preview-deployment-result.json`, packet validator, deployment smoke reports | Pinned deployment and temporary Access setup/cleanup passed; one no-retry smoke produced no HTTP response |
