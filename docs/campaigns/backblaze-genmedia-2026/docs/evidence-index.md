...
| Preview deployment trust boundary and judge procedure are reviewable | PARTIAL CLOUD | `preview-runtime-decision.md`, threat model, judge runbook, `preview-deployment-result.json` | Access and rollback are proven; authenticated cloud B2 transaction and judge path remain blocked |
| Cloudflare provenance runtime is source-bound and reproducible locally | LOCAL B2 + GUARDED CLOUD HEALTH | `preview-runtime-plan.json`, same-origin Pages Function, exact retained-source binding, focused Worker tests, `2026-07-23-cloudflare-preview-deployment-smoke.md` | Cloud health passed and equivalent local B2 transaction passed once; cloud B2 transaction returned HTTP 502 |
| Preview cloud configuration, smoke, and rollback handoff are machine checked | EXECUTED THEN ROLLED BACK | `preview-deployment-packet.json`, `preview-deployment-result.json`, packet validator, deployment smoke report | Access passed; temporary smoke UI was removed and feature disabled after cloud B2 HTTP 502 |
